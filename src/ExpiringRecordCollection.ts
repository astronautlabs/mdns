...
export class ExpiringRecordCollection extends EventEmitter {
  ...
  private _records: Record<string, ResourceRecord>;
  private _related: Record<string, Set<string>>;
  private _insertionTime: Record<string, number>;
  private _timerContainers: Record<string, TimerContainer>;
  size = 0;
  ...
  add(record: ResourceRecord) {
    ...
    // [debug.verbose]: #add(): ${record}
    // [debug.verbose]:     to: ${this._desc}
  ...
  addEach(records: ResourceRecord[]) {
    records.forEach(record => this.add(record));
  }


  has(record: ResourceRecord) {
   ...
   * @return {boolean}
   */
  hasAddedWithin(record: ResourceRecord, range: number) {
    const then = this._insertionTime[record.hash];

    return Number(then) === then &&
  ...
  delete(record: ResourceRecord) {
    ...
    // [debug.verbose]: deleting: ${record}
    // [debug.verbose]:     from: ${this._desc}
   ...
   */
  setToExpire(record: ResourceRecord) {
    ...
    // [debug.verbose]: #setToExpire(): ${record}
    // [debug.verbose]:             on: ${this._desc}
   ...
   * longer than 1s.
   */
  flushRelated(record: ResourceRecord) {
   ...
   * @return {boolean}
   */
  hasConflictWith(record: ResourceRecord) {
   ...
   */
  find(query: QueryRecord, cutoff = 0.25) {
    // [debug.verbose]: #find(): "${query.name}" type: ${query.qtype}
    // [debug.verbose]:      in: ${this._desc}
  ...
  getAboveTTL(cutoff = 0.25) {
    // [debug.verbose]: #getAboveTTL(): %${cutoff * 100}
   ...
   */
  _getRelatedRecords(namehash: string) {
   ...
   * @return {ResouceRecords[]}
   */
  _filterTTL(records: ResourceRecord[], cutoff: number) {
    return records.reduce((result, record) => {
      const then = this._insertionTime[record.hash];
      const elapsed = ~~((Date.now() - then) / ONE_SECOND);
      const percent = (record.ttl - elapsed) / record.ttl;

      // [debug.verbose]: └── ${record} @ ${~~(percent*100)}%'
   ...
   */
  _schedule(record: ResourceRecord) {
    const id = record.hash;
