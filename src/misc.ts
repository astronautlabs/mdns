 ...
 * @return {object}
 */
export function parse(fullname) {
  const obj: {
    protocol?: '_tcp' | '_udp';
    tld?: string;
    service?: string;
...
export function pad(value, len, fill = ' ') {
...
export function padStart(value, len, fill = ' ') {
  const str = String(value);
  const needed = len - str.length;
 ...
 * @return {string}
 */
export function visualPad(str, num) {
 ...
 * @return {string[][]}
 */
export function alignRecords(...groups) {
  const colWidths = [];
  let result;
 ...
 */
export function makeRawTXT(obj: Record<string, boolean | string | Buffer | number>): Record<string, true | Buffer> {
  const result = {};
 ...
 * @return {object} - a new object, original not modified
 */
export function makeReadableTXT(obj: Record<string, boolean | number | string | Buffer>): Record<string, true | string> {
  const result = {};
...
export function color(str, color = 'white', bright = false) {
  const colors = {
    black:   30,
    red:     31,
    green:   32,
...
export function bg(str, color = 'white', bright = false) {
  const colors = {
    black:   40,
    red:     41,
    green:   42,
...
export function truncate(str, len, end = '…') {
 ...
 * Map fn() n times
 */
export function map_n<T>(fn: () => T, n: number): T[] {
  const results = [];
