 ...
 */
export class Responder extends StateMachine {
  constructor(intf: NetworkInterface, records: ResourceRecord[], bridgeable?: ResourceRecord[]) {
    super();
  ...
  states = {
    ...
    stopped: {
      enter: (err) => {
        ...
        this.transition = () => {
          // [debug]: Responder is stopped! Can't transition.
        };
      },
    },
  };
  ...
  _bridgeable: RecordCollection<ResourceRecord>;
  _fullname: string;
  _instance: string;
  _timers = new TimerContainer();
  _conflicts = new ConflictCounter();
  ...
  _addListeners() {
    this._interface.on('probe', this._onProbeHandler = packet => this.handle('probe',  packet))
    this._interface.on('query', this._onQueryHandler = packet => this.handle('query',  packet))
    this._interface.on('answer', this._onAnswerHandler = packet => this.handle('answer', packet))
  ...
  private _removeListeners() {
    if (this._onProbeHandler) this._interface.off('probe', this._onProbeHandler);
    if (this._onQueryHandler) this._interface.off('query', this._onQueryHandler);
  ...
  protected createProbe() {
    return new Probe(this._interface, this._offswitch);
  }
  ...
  _onQuery(packet) {
    ...
    const name = this._fullname;
    const records = this._records;
    const knownAnswers = new RecordCollection<ResourceRecord>(packet.answers);
    ...
    if (suppressed.length) {
      // [debug]: Suppressing known answers (${this._id}): ${suppressed}
    }
  ...
  protected createGoodbyeResponse() {
    return new GoodbyeResponse(this._interface, this._offswitch);
  }

  protected createMulticastResponse() {
    return new MulticastResponse(this._interface, this._offswitch);
  }

  protected createUnicastResponse() {
    return new UnicastResponse(this._interface, this._offswitch);
  }
  ...
  _onAnswer(packet) {
    ...
    const incoming = new RecordCollection<ResourceRecord>([...packet.answers, ...packet.additionals]);
