...
const RClasses = {
  ...
  PTRRecord  : RType.PTR,
  TXTRecord  : RType.TXT,
  AAAARecord : RType.AAAA,
  SRVRecord  : RType.SRV,
  NSECRecord : RType.NSEC,
  ANYRecord  : RType.ANY,
};
 ...
 * > const record = new AAAARecord({name: 'Target.local.', address: '::1'});
 *
...
export abstract class ResourceRecord {
    constructor(fields: ResourceRecordInit) {
        if (!fields?.name)
            throw new Error('Record must have a name');

        this.name = fields.name;
        this.rrtype = fields.rrtype || RClasses[this.constructor.name];
    ...
    static fromBuffer(wrapper) {
        ...
        const fields: ResourceRecordInit = {
            name,
            rrtype,
        ...
        if (rrtype === RType.A) return new ARecord(fields, wrapper);
        if (rrtype === RType.PTR) return new PTRRecord(fields, wrapper);
        if (rrtype === RType.TXT) return new TXTRecord(fields, wrapper);
        if (rrtype === RType.AAAA) return new AAAARecord(fields, wrapper);
        if (rrtype === RType.SRV) return new SRVRecord(fields, wrapper);
        if (rrtype === RType.NSEC) return new NSECRecord(fields, wrapper);

        return new UnknownResourceRecord(fields, wrapper);
    }
    ...
    protected abstract _getRDataStr(): string;
    protected abstract _writeRData(wrapper: BufferWrapper);
    protected abstract _hashRData(): string;
...
export interface ARecordInit extends ResourceRecordInit {
    address?: string;
}
...
export class ARecord extends ResourceRecord {
     ...
     */
    constructor(fields: ARecordInit, wrapper?: BufferWrapper) {
        super({
            ttl: 120,
            isUnique: true,
            ...fields
    ...
    _writeRData(wrapper: BufferWrapper) {
...
export class PTRRecord extends ResourceRecord {
    constructor(fields, wrapper?: BufferWrapper) {
        super({
            ttl: 4500,
            isUnique: false,
            ...fields 
...
export class AAAARecord extends ResourceRecord {
    constructor(fields: AAAARecordInit, wrapper?: BufferWrapper) {
        super({ 
            ttl: 120,
            isUnique: true,
            ...fields 
...
export class SRVRecord extends ResourceRecord {
    constructor(fields: SRVRecordInit, wrapper?: BufferWrapper) {
        super({ 
            ttl: 120,
            isUnique: true,
            ...fields 
...
export interface NSECRecordInit extends ResourceRecordInit {
    existing?: number[];
}
...
export class NSECRecord extends ResourceRecord {
    constructor(fields: NSECRecordInit, wrapper?: BufferWrapper) {
        super({ 
            ttl: 120,
            isUnique: true,
            ...fields
...
export interface UnknownResourceRecordInit extends ResourceRecordInit {
    rdata: Buffer;
}
 ...
 */
export class UnknownResourceRecord extends ResourceRecord {
    constructor(fields, wrapper?: BufferWrapper) {
        super({ 
            ttl: 120,
            isUnique: true,
    ...
    _readRData(wrapper: BufferWrapper) {
        const rdataLength = wrapper.readUInt16BE();
