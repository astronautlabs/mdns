...
class ServiceResolver extends StateMachine {
   ...
   */
  _addListeners() {
    this._interface.using(this).on('answer', this._onAnswer).once('error', err => this.transition('stopped', err));
   ...
   */
  _onReissue(record) {
    const isRelevant = record.matches({ name: this.fullname }) || record.matches({ name: this.ptrname, PTRDName: this.fullname }) || record.matches({ name: this.target });
  ...
  _onExpired(record) {
    ...
    const isDown = record.matches({ rrtype: RType.SRV, name: this.fullname }) || record.matches({ rrtype: RType.PTR, name: this.ptrname, PTRDName: this.fullname });

    // A/AAAA: remove address & transition to unresolved if none are left
    const isAddress = record.matches({ rrtype: RType.A, name: this.target }) || record.matches({ rrtype: RType.AAAA, name: this.target });
