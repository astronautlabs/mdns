...
validate.serviceName = function serviceName(str) {
  if (typeof str !== 'string') {
    throw new ValidationError('Service name must be a string, got %s', typeof str);
  }
...
validate.label = function label(str, name = 'label') {
  if (typeof str !== 'string') {
    throw new ValidationError('%s name must be a string, got %s', name, typeof str);
  }
...
validate.txt = function txt(obj) {
  ...
  Object.keys(obj).forEach(key => {
    ...
    if (typeof value !== 'string' && typeof value !== 'boolean' && !isNumeric(value) && !Buffer.isBuffer(value)) {
      throw new ValidationError('TXT values must be a string, buffer, number, or boolean. got %s', typeof value);
    }
