...
if (process.env.DEBUG) {
  process.env.DEBUG.replace(/\*/g, '.*?').split(',').filter(s => !!s).forEach(namespace => {
    namespace.substr(0, 1) === '-' ? disabledNamespaces.push(namespace.substr(1)) : enabledNamespaces.push(namespace);
  });
}

if (process.env.VERBOSE) {
  process.env.VERBOSE.replace(/\*/g, '.*?').split(',').filter(s => !!s).forEach(namespace => {
...
function timestamp() {
  ...
  const time = [misc.padStart(now.getHours(), 2, '0'), misc.padStart(now.getMinutes(), 2, '0'), misc.padStart(now.getSeconds(), 2, '0'), misc.padStart(now.getMilliseconds(), 3, '0')];
