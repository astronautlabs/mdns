...
MulticastResponse.prototype._suppressRecents = function (packet) {
  ...
  const answers = packet.answers.filter(record => !this._intf.hasRecentlySent(record, range));
...
UnicastResponse.prototype._makePacket = function () {
  ...
  let additionals = answers.reduce((result, answer) => result.concat(answer.additionals), []).filter(add => !~answers.indexOf(add));
  ...
  if (this._isLegacy) {
    ...
    answers = answers.filter(record => record.rrtype !== RType.NSEC).map(legacyify);

    additionals = additionals.filter(record => record.rrtype !== RType.NSEC).map(legacyify);
  }
