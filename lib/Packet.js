...
class Packet {
  ...
  writeHeader(wrapper) {
    const flags = 0 + (this.header.QR << 15) + (this.header.OPCODE << 11) + (this.header.AA << 10) + (this.header.TC << 9) + (this.header.RD << 8) + (this.header.RA << 7) + (this.header.Z << 6) + (this.header.AD << 5) + (this.header.CD << 4) + this.header.RCODE;
  ...
  isLocal() {
    return !!this.origin.address && [].concat(...Object.values(os.networkInterfaces())).some(({ address }) => address === this.origin.address);
  }
  ...
  toString() {
    ...
    const recordGroups = [];
    const aligned = misc.alignRecords(this.questions, this.answers, this.authorities, this.additionals);
