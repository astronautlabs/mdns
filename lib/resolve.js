...
function runQuery(name, qtype, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function sendQuery() {
      new Query(intf, killswitch).continuous(false).setTimeout(timeout).add(question).once('answer', (answer, related) => {
        stop();
...
function resolve4(name, opts) {
  return resolveAny(name, 'A', opts).then(result => result.answer.address);
}

function resolve6(name, opts) {
  return resolveAny(name, 'AAAA', opts).then(result => result.answer.address);
}

function resolveSRV(name, opts) {
  return resolveAny(name, 'SRV', opts).then(result => ({ target: result.answer.target, port: result.answer.port }));
}

function resolveTXT(name, opts) {
  return resolveAny(name, 'TXT', opts).then(result => ({ txt: result.answer.txt, txtRaw: result.answer.txtRaw }));
}
