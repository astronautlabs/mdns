...
module.exports.parse = function (fullname) {
  ...
  if (!!~fullname.indexOf('._tcp.') || !!~fullname.indexOf('._udp.')) {
    ...
    const parts = fullname.split(obj.protocol).map(part => part.split('.').filter(p => !!p));
...
function alignRecords(...groups) {
  ...
  // Add padding:
  result = result.map(records => records.map(recordParts => recordParts.map((part, i) => visualPad(part, colWidths[i])).join(' ')));
...
module.exports.makeRawTXT = function (obj) {
  ...
  Object.keys(obj).filter(key => obj[key] !== false).forEach(key => {
    let value = obj[key];
...
module.exports.format = function (msg, ...args) {
  ...
  let output = hasFormatters && args.length ? msg.replace(/%([a-z])/g, type => stringify(args.shift(), type)) : stringify(msg);
