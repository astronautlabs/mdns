...
function findInterfaceName(address) {
  ...
  return Object.keys(interfaces).find(name => interfaces[name].some(addr => addr.address === address));
}
...
NetworkInterface.prototype._bindSocket = function () {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    socket.on('error', err => {
      if (isPending) reject(err);else this._onError(err);
    });
    ...
    socket.on('listening', () => {
      ...
      const addresses = [].concat(...Object.values(os.networkInterfaces())).filter(addr => addr.family === 'IPv4').map(addr => addr.address);
