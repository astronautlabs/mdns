...
class ExpiringRecordCollection extends EventEmitter {
  ...
  hasConflictWith(record) {
    ...
    return !!this._getRelatedRecords(record.namehash).filter(related => !related.equals(record)).length;
  }
  ...
  _getRelatedRecords(namehash) {
    return this._related[namehash] && this._related[namehash].size ? [...this._related[namehash]].map(id => this._records[id]) : [];
  }
