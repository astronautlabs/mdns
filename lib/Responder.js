...
const responderStates = {
  probing: {
    enter() {
      ...
      const onSuccess = early => {
        this.transition('responding', early);
      };
      const onFail = () => {
...
class Responder extends StateMachine {
  ...
  updateEach(rrtype, fn) {
    ...
    this._records.filter(record => record.rrtype === rrtype).forEach(record => record.updateWith(fn));

    // (update bridge list too)
    this._bridgeable.filter(record => record.rrtype === rrtype).forEach(record => record.updateWith(fn));
  ...
  _addListeners() {
    this._interface.using(this).on('probe', packet => this.handle('probe', packet)).on('query', packet => this.handle('query', packet)).on('answer', packet => this.handle('answer', packet)).once('error', err => this.transition('stopped', err));
  }
