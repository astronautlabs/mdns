...
class ResourceRecord {
   ...
   */
  writeTo(wrapper) {
    const classField = this.isUnique ? this.rrclass | 0x8000 : this.rrclass;
  ...
  conflictsWith(record) {
    const hasConflict = this.isUnique && record.isUnique && this.namehash === record.namehash && this.rdatahash !== record.rdatahash;
   ...
   */
  matches(properties) {
    return Object.keys(properties).map(key => [key, properties[key]]).every(([key, value]) => {
      return typeof this[key] === 'string' && typeof value === 'string' ? this[key].toUpperCase() === value.toUpperCase() : misc.equals(this[key], value);
    });
  }
  ...
  toParts() {
    ...
    const type = this.constructor.name === 'Unknown' ? this.rrtype : this.constructor.name;
