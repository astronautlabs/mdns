...
Advertisement.prototype.start = function (callback) {
  ...
  this._interface.bind().then(() => this._getDefaultID()).then(() => this._advertiseHostname()).then(() => this._advertiseService()).catch(err => this._onError(err)).then(err => {
    if (callback) {
...
Advertisement.prototype._restart = function () {
  ...
  this._getDefaultID().then(() => this._advertiseHostname()).then(() => this._advertiseService()).catch(err => this._onError(err));
};
...
Advertisement.prototype._makeAddressRecords = function (addresses) {
  ...
  const As = addresses.filter(({ family }) => family === 'IPv4').map(({ address }) => new ResourceRecord.A({ name, address, ttl: this.hostTTL }));

  const AAAAs = addresses.filter(({ family }) => family === 'IPv6').filter(({ address }) => address.substr(0, 6).toLowerCase() === 'fe80::').map(({ address }) => new ResourceRecord.AAAA({ name, address, ttl: this.hostTTL }));
