...
Browser.prototype.start = function () {
  ...
  this._interface.bind().then(() => this._startQuery()).catch(err => this._onError(err));
...
Browser.prototype.list = function () {
  ...
  return Object.values(this._resolvers).filter(resolver => resolver.isResolved()).map(resolver => resolver.service());
};
...
Browser.prototype._startQuery = function () {
  ...
  const answerHandler = this._isWildcard ? this._addServiceType.bind(this) : this._addService.bind(this);
